...
weights_file <- "test5.txt"

#coordinates file for each window
window_data_file <- "output.unphased.phyml_bionj.w50.data.tsv"
...
pdf(file = paste0("5species.raw.stepped.stacked.w50.pdf"), width = 10, height = 4)
par(mar = c(4,4,1,1))
...
pdf(file = paste0("5species.raw.stepped.unstacked.w50.pdf"), width = 10, height = 4)
par(mar = c(4,4,1,1))
plot_weights(weights_dataframe=weights, positions=cbind(window_data$start,window_data$end),
...
pdf(file = paste0("5species.raw.midpoints.unstacked.w50.pdf"), width = 10, height = 4)
par(mar = c(4,4,1,1))
...
pdf(file = paste0("5species.smooth.stacked.w50.pdf"), width = 10, height = 4)
par(mar = c(4,4,1,1))
...
pdf(file = paste0("5species.smooth.unstacked.w50.pdf"), width = 10, height = 4)
par(mar = c(4,4,1,1))
plot_weights(weights_dataframe=weights_smooth, positions=window_data$mid,
             line_cols=cols, fill_cols=trans_cols, xlim =c(1,100000),stacked=FALSE)
dev.off()
...
topos = read.tree(file="5species.topologies.trees")

#unrooted trees

pdf(file = paste0("5species.topos.unrooted.w50.pdf"), width = 5, height = 2)
...
#root trees
for (i in 1:length(topos)) topos[[i]] <- root(topos[[i]], "ptri", resolve.root = T)

pdf(file = paste0("5species.topos.rooted.w50.pdf"), width = 5, height = 2)
par(mfrow = c(1,3), mar = c(1,1,2,1), xpd=NA)
for (n in 1:length(topos)){
